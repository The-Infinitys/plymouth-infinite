...
def generate_frame_data(frame_index: int, total_frames: int) -> tuple[str, str, str, str]:
    """
    ...
    cycle_norm = (
        (frame_index / total_frames) * PATH_CYCLE_LENGTH * ANIMATION_CYCLE_MULTIPLIER
    )
    ...
    color1 = COLORS_CYCLE[int(len(COLORS_CYCLE) / ANIMATION_CYCLE_MULTIPLIER * color_idx) % len(COLORS_CYCLE)]
    color2 = COLORS_CYCLE[int(len(COLORS_CYCLE) / ANIMATION_CYCLE_MULTIPLIER * color_idx + 1) % len(COLORS_CYCLE)]
    color3 = COLORS_CYCLE[int(len(COLORS_CYCLE) / ANIMATION_CYCLE_MULTIPLIER * color_idx + 2) % len(COLORS_CYCLE)]
    ...
    stroke_color3 = interpolate_color(WHITE_HEX, color3, t_color)

    return path_d, stroke_color1, stroke_color2, stroke_color3


def generate_svg_file(frame_index: int, total_frames: int, output_dir: str):
    """単一のSVGファイルを生成して保存します。"""

    path_d, stroke_color1, stroke_color2, stroke_color3 = generate_frame_data(
...
<svg
    ...
    <defs>
        ...
        <linearGradient id="animated-gradient" x1="0%" y1="0%" x2="100%" y2="0%">
            ...
            <stop offset="50%" stop-color="{stroke_color2}" />
            <stop offset="100%" stop-color="{stroke_color3}" />
        </linearGradient>
    </defs>
